...
async def main():
  async with async_playwright() as p:
    ...
    await pg.click("#openerSkip"); await pg.wait_for_timeout(100)
    ok("Skip ends opener, focus to Begin Practice",not await pg.query_selector("#opener") and await pg.evaluate("document.activeElement.id")=="beginBtn" and not await pg.evaluate("document.querySelector('#welcome').inert"))
    p2=await b.new_page(); await p2.goto(URL); await p2.wait_for_timeout(5400)
    ok("opener ends on its own after ~5 s",not await p2.query_selector("#opener") and await p2.evaluate("document.activeElement.id")=="beginBtn")
    await p2.close()
