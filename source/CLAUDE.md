...
- `app.js`: all logic, in an IIFE.
  ...
  - `playOpener` is the Miva opener (`#opener` in `body.html`, `op-*` styles): ~5 s on load, Skip/Escape end it, the welcome screen is `inert` meanwhile. When it ends it calls `openPlayer(beginBtn,{auto:true})`: the walkthrough plays, falling back to muted with a `#vUnmute` button when the browser blocks sound (no click yet). Not under reduced motion (opens paused), and not when the build has no video. `__sandbox.skipOpener()` ends it without opening the walkthrough, for tests and the recorder.
...
- `test/verify.py`: Playwright functional suite (70 checks). Build first; the tests load `dist/local.html`. `test/player.py` checks the walkthrough player.
...
- `brand/`: the Miva logos. Blue on light backgrounds, white on dark. `build.py` inlines them for the `@@LOGO_BLUE@@` / `@@LOGO_WHITE@@` tokens in `body.html` and `app.js`.
