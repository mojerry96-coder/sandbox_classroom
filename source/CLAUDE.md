...
- `app.js`: all logic, in an IIFE.
  ...
  - `playOpener` is the Miva opener (`#opener` in `body.html`, `op-*` styles): ~5 s on load, Skip/Escape end it, the welcome screen is `inert` meanwhile, focus then goes to Begin Practice. `__sandbox.skipOpener()` ends it for tests and the recorder.
...
- `test/verify.py`: Playwright functional suite (66 checks). Build first; the tests load `dist/local.html`. `test/player.py` checks the walkthrough player.
