...
- `app.js`: all logic, in an IIFE.
  ...
  - Classwork (phase 3, SPEC §6.6/§6.7): `WORK` describes the four editor types; `openEditor(type,a,trigger)` builds the full-screen editor — title in its error state on open, instructions with the toolbar, the five-button attach card, and the right rail (For, Assign to, Points combobox with the Ungraded suggestion, Due popover, Topic select with Create topic, Rubric). `duePopover`, `reusePost` and `firstRunPromo` sit beside it.
  - **Layering matters here**: the editor is z-index 85, dialogs 100, menus 110. A dialog opened from inside the editor (Create topic, Announce to) and a menu opened from inside a dialog (the Post/Assign split) must sit above their parent, or they are unclickable. Two checks guard this.
  - States (phase 6): `bootSkeleton` shows the product's loading frame when the shell first opens; `routeBar` is the 4px indeterminate bar under the top app bar on every navigation; `BANNERS` / `showBanner` / `syncBanners` are the cross-cutting banners (archived and co-teach are state-driven and re-derived, refresh and stream-updated are transient and stay until dismissed). `__sandbox.banner(kind)` raises one for tests and the recorder.
  - The other views (phase 5): `VIEWS` holds Calendar, To-do, To review, Archived classes and Settings; `renderView` draws them, with `UI.viewTab` for the tabbed ones. They read from `S.classes`, so they stay true as the learner builds. Home has both role views, the class card carries its overflow menu (Edit / Copy / Archive for a teacher, Hide / Unenroll for a student), and archived classes drop out of Home and the drawer until restored.
  - People (phase 4, SPEC §6.8): `inviteStudents` is the invite dialog (invite link + copy, searchable `CONTACTS`, Invite disabled until someone is picked). Invited people carry `status:"invited"` and show the Invited badge until the practice shortcut in their row menu accepts for them. `openAddStudent` remains the shortcut that adds someone straight in as Active.
...
- `test/verify.py`: Playwright functional suite (146 checks). Build first; the tests load `dist/local.html`. `test/player.py` checks the walkthrough player.
