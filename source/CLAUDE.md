...
- `brand/`: the Miva logos (blue on light, white on dark), the opener artwork (`ekiti.svg`, `miva.svg`, `tof.svg`, `chevrons.svg`) and the house spec `OPENER.md`. `build.py` inlines all of them for the `@@LOGO_*@@` tokens in `body.html`, `app.js` and `styles.css` (the chevron motif is a CSS background).
