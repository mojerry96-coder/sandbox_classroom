...
function seed(){
  ...
  L={role:null,classesCreated:0,classesJoined:0,classesArchived:0,
     ...
     nudgeShown:false,
     invitesSent:0,invitesCancelled:0,studentsInvited:0,invitesAccepted:0,studentsAdded:0,studentsRemoved:0,codeRegenerations:0};
...
/* ======================================================================
   ...
   Views over the same four nouns, so they read from the classes that exist.
   ====================================================================== */
const VIEWS={
 "Calendar":{icon:"calendar_today",build:()=>{
   const work=S.classes.filter(c=>!c.archived).flatMap(c=>c.assignments.filter(a=>a.due).map(a=>({c,a})));
   const days=["Mon","Tue","Wed","Thu","Fri"];
   return `<div class="vhead"><h1>Calendar</h1><button class="ob" id="vwFilter">${I("filter_list")}All classes</button></div>
    <div class="cal"><div class="calrow head">${days.map(d=>`<div>${d}</div>`).join("")}</div>
     <div class="calrow">${days.map((d,i)=>`<div class="calcell">${work.filter((_,k)=>k%5===i).map(({c,a})=>`<span class="chip" title="${esc(c.name)}">${esc(excerpt(a.title,18))}</span>`).join("")}</div>`).join("")}</div></div>
    ${work.length?"":`<div class="cw-empty small">${EMPTY_RECENT}<b>Nothing scheduled</b><span>Work with a due date appears here. Set a due date on an assignment to see it.</span></div>`}`}},
 "To-do":{icon:"task_alt",tabs:["Assigned","Missing","Done"],build:(tab)=>{
   const work=S.classes.filter(c=>c.role==="student"&&!c.archived).flatMap(c=>c.assignments.map(a=>({c,a})));
   return listView(work,tab==="Assigned"?"You don’t have any work assigned":tab==="Missing"?"No missing work. Nice.":"Nothing turned in yet")}},
 "To review":{icon:"checklist",tabs:["To review","Reviewed"],build:(tab)=>{
   const work=S.classes.filter(c=>c.role==="owner"&&!c.archived).flatMap(c=>c.assignments.map(a=>({c,a})));
   return tab==="To review"?listView(work,"No work to review yet. Student work would appear here."):listView([],"Nothing reviewed yet")}},
 "Archived classes":{icon:"archive",build:()=>{
   const arch=S.classes.filter(c=>c.archived);
   return arch.length?`<div class="vhead"><h1>Archived classes</h1></div><ul class="grid">${arch.map(cardHTML).join("")}</ul>`
     :`<div class="vhead"><h1>Archived classes</h1></div><div class="cw-empty">${EMPTY_ARCHIVE}<b>None of your classes have been archived</b>
       <div style="margin-top:16px"><button class="tb" id="vwWhat">What does this mean?</button></div></div>`}},
 "Settings":{icon:"settings",build:()=>`<div class="vhead"><h1>Settings</h1></div>
   <section class="scard"><h2>Profile</h2>
     <div class="setrow"><div><b>Profile picture</b><span>Shown beside everything you post</span></div><button class="ob" data-note="Profile pictures aren't part of this practice.">Change</button></div>
     <div class="setrow"><div><b>Account settings</b><span>Change your password and security options, and access other Google services.</span></div><button class="ob" data-note="Account settings aren't part of this practice.">Manage</button></div>
     <div class="setrow"><div><b>Change name</b><span>To change your name, go to your account settings.</span></div></div>
   </section>
   <section class="scard"><h2>Notifications</h2>
     ${[["Email","Allow email notifications"],["Comments","Comments on your posts|Comments that mention you|Private comments on work"],
        ["Classes you're enrolled in","Work and other posts from teachers|Returned work and grades from your teachers|Invitations to join classes as a student|Due-date reminders for your work"],
        ["Classes you teach","Late submissions of student work|Resubmissions of student work|Invitations to co-teach classes|Scheduled post published or failed"],
        ["Class notifications","Notifications for each class you are in"]]
       .map(([grp,items])=>`<div class="notegrp"><h3>${grp}</h3>${items.split("|").map(l=>`<label class="swrow"><span>${l}</span><input type="checkbox" class="sw" checked data-note="Notification settings are simulated; no mail is ever sent."></label>`).join("")}</div>`).join("")}
   </section>`}
};
function listView(items,emptyLine){
  return items.length?`<ul class="worklist">${items.map(({c,a})=>`<li><span class="icon">${I(workOf(a).icon)}</span><span class="t">${esc(a.title)}<small>${esc(c.name)}</small></span><span class="d">${a.due?esc(a.due):"No due date"}</span></li>`).join("")}</ul>`
    :`<p class="emptyline">${emptyLine}</p>`;
}
const EMPTY_ARCHIVE=`<svg width="200" height="140" viewBox="0 0 200 140" aria-hidden="true"><rect x="40" y="40" width="120" height="70" rx="10" fill="#E9EEF6"/><rect x="40" y="40" width="120" height="20" rx="10" fill="#DDE3EA"/><rect x="82" y="72" width="36" height="8" rx="4" fill="#A8C7FA"/></svg>`;
function renderView(m,name){
  const V=VIEWS[name];
  if(!V){m.innerHTML=`<div class="sheet"><div class="placeholder-page">${I("explore")}<b>${esc(name)} isn't part of this practice</b></div></div>`;return}
  const tabs=V.tabs||null;
  if(tabs&&!tabs.includes(UI.viewTab))UI.viewTab=tabs[0];
  m.innerHTML=`<div class="sheet"><div class="viewpage">
    ${tabs?`<div class="vhead"><h1>${esc(name)}</h1></div><div class="ctabs" role="tablist" aria-label="${esc(name)}">
      ${tabs.map(t=>`<button class="ctab" role="tab" id="vt-${t.replace(/\W/g,"")}" data-vtab="${esc(t)}" aria-selected="${UI.viewTab===t}" tabindex="${UI.viewTab===t?0:-1}">${esc(t)}</button>`).join("")}</div>`:""}
    <div class="vbody">${V.build(UI.viewTab)}</div></div></div>`;
  $$("[data-vtab]").forEach(b=>b.onclick=()=>{UI.viewTab=b.dataset.vtab;renderView(m,name)});
  $$("[data-note]").forEach(b=>b.onclick=()=>snack(b.dataset.note));
  $$(".sw").forEach(sw=>sw.onchange=()=>snack("Notification settings are simulated; no mail is ever sent."));
  const f=$("#vwFilter");if(f)f.onclick=()=>snack("Filtering the calendar isn't part of this practice.");
  const wt=$("#vwWhat");if(wt)wt.onclick=()=>snack("Archiving keeps a class out of Home without deleting it.");
  $$("[data-open-class]").forEach(b=>b.onclick=e=>{e.preventDefault();openClass(b.dataset.openClass,"stream")});
  $$("[data-card-menu]").forEach(b=>b.onclick=e=>{e.stopPropagation();const c=S.classes.find(x=>x.id===b.dataset.cardMenu);
    openMenu(b,[{icon:"archive",label:"Restore",fn:()=>{c.archived=false;goHome();snack(`${c.name} restored.`)}}],{alignRight:true})});
}
...
function renderHome(m){
  ...
  const section=(id,title,info,body,link)=>`<section class="scard" aria-labelledby="${id}">
    <div class="shead"><h2 id="${id}">${title}</h2>${info?`<span class="sinfo">${info}</span>`:""}<span class="sp"></span>${link?`<button class="tb" data-home-link="${esc(link)}">${esc(link)}</button>`:""}</div>
    ${body}</section>`;
  m.innerHTML=`<div class="home">
   ${both?`<div class="seg" role="tablist" aria-label="Class role">
     <button role="tab" id="segTeaching" aria-selected="${view==="teaching"}" data-home="teaching">${view==="teaching"?I("check"):""}Teaching</button>
     <button role="tab" id="segEnrolled" aria-selected="${view==="enrolled"}" data-home="enrolled">${view==="enrolled"?I("check"):""}Enrolled</button></div>`:""}
   ${view==="teaching"
     ? section("rdH","Recently due","","<div class=\"cw-empty small\">"+EMPTY_RECENT+"<b>No recent assignments to review</b><span>See assignments from the past 7 days that need review</span></div>","View work in progress")
     : section("dsH","Due soon","","<p class=\"emptyline\">You don't have any work due in the next 7 days</p>","View To-do")
       +section("cltH","Class learning tools","Gems and Notebooks that have been recently posted by your teachers","<p class=\"emptyline\">You don’t have any assigned class learning tools</p>","")}
   ${section("clsH","Classes","","",teacher?"Add class":"")}
  ...
  $$("[data-home-link]").forEach(b=>b.onclick=()=>{const l=b.dataset.homeLink;
    if(l==="Add class")return startCreateClass(b);
    if(l==="View To-do"){UI.view="placeholder";UI.placeholder="To-do";return renderAll()}
    if(l==="View work in progress"){UI.view="placeholder";UI.placeholder="To review";return renderAll()}});
  $$("[data-open-class]").forEach(b=>b.onclick=e=>{e.preventDefault();openClass(b.dataset.openClass,"stream")});
  $$("[data-card-tab]").forEach(b=>b.onclick=()=>openClass(b.dataset.cardClass,b.dataset.cardTab));
  $$("[data-card-menu]").forEach(b=>b.onclick=e=>{e.stopPropagation();const c=S.classes.find(x=>x.id===b.dataset.cardMenu);
    openMenu(b,c.role==="owner"
      ? [{icon:"edit",label:"Edit",fn:()=>{openClass(c.id,"stream");setTimeout(()=>classSettings($("#tbSettings")),80)}},
         {icon:"content_copy",label:"Copy",fn:()=>snack("Copying a class isn't part of this practice.")},
         {icon:"archive",label:"Archive",fn:()=>archiveClass(c)}]
      : [{icon:"visibility_off",label:"Hide",fn:()=>snack("Hiding a class isn't part of this practice.")},
         {icon:"person_remove",label:"Unenroll",fn:()=>unenrol(c)}],{alignRight:true})});
}
const EMPTY_RECENT=`<svg width="170" height="120" viewBox="0 0 170 120" aria-hidden="true"><rect x="30" y="30" width="110" height="64" rx="8" fill="#E9EEF6"/><rect x="44" y="46" width="48" height="7" rx="3.5" fill="#A8C7FA"/><rect x="44" y="62" width="82" height="6" rx="3" fill="#fff"/><rect x="44" y="76" width="60" height="6" rx="3" fill="#fff"/></svg>`;
function archiveClass(c){
  cDialog(`<h2 id="cdT">Archive ${esc(c.name)}?</h2><p>You and your students will no longer see this class in Home. You can find it under Archived classes.</p>
   <div class="acts"><button class="tb" data-close>Cancel</button><button class="tb" id="arGo">Archive</button></div>`,
  (d,close)=>{d.querySelector("#arGo").onclick=()=>{c.archived=true;if(A&&A.id===c.id){A=null;S.activeId=null}
    L.classesArchived++;close(true);goHome();snack(`${c.name} archived.`)}});
}
function unenrol(c){
  cDialog(`<h2 id="cdT">Unenroll from ${esc(c.name)}?</h2><p>You will be removed from the class and will no longer see its posts or work.</p>
   <div class="acts"><button class="tb" data-close>Cancel</button><button class="tb" id="unGo">Unenroll</button></div>`,
  (d,close)=>{d.querySelector("#unGo").onclick=()=>{S.classes=S.classes.filter(x=>x!==c);if(A&&A.id===c.id){A=null;S.activeId=null}
    close(true);goHome();snack(`Unenrolled from ${c.name}.`)}});
}
function cardHTML(c){
  return `<li class="ccard"><div class="cban"><canvas class="cvs" aria-hidden="true" style="position:absolute;inset:0;width:100%;height:100%"></canvas>
   ...
   <button class="ib cmenu" data-card-menu="${c.id}" aria-haspopup="menu" aria-expanded="false" aria-label="Class options for ${esc(c.name)}">${I("more_vert")}</button>
   <span class="cav" aria-hidden="true">${esc((c.role==="owner"?"Y":c.teachers[0]?c.teachers[0].name:"T").trim()[0].toUpperCase())}</span>
   <div class="cbody"><p class="due">${c.assignments&&c.assignments.length?"No work due soon":"Woohoo, no work due soon!"}</p></div>
...
const CONTACTS=[["Ada Okafor","ada.okafor@example.com"],["Chidi Nwosu","chidi.nwosu@example.com"],["Amina Bello","amina.bello@example.com"],
  ["Tobi Adeyemi","tobi.adeyemi@example.com"],["Ngozi Eze","ngozi.eze@example.com"],["Emeka Obi","emeka.obi@example.com"],
  ["Fatima Sani","fatima.sani@example.com"],["Kelechi Umeh","kelechi.umeh@example.com"]];
function renderPeople(p){
  const n=A.students.length,active=A.students.filter(s=>s.status!=="invited").length;
  const srow=s=>`<div class="prow"><span class="la" style="background:${laColor(s.name)}" aria-hidden="true">${esc(s.name.trim()[0].toUpperCase())}</span>
     <div class="n">${esc(s.name)}<small>${s.status==="invited"?"Invited · hasn't joined yet":(s.placeholder?"Fictional placeholder student":"Fictional student added by you")}</small></div>
     ${s.status==="invited"?`<span class="badge b-pending">${I("schedule")}Invited</span>`:`<span class="badge b-active">${I("check")}Active</span>`}
     ${owner()?`<button class="ib" data-student-menu="${s.id}" aria-haspopup="menu" aria-expanded="false" aria-label="Options for ${esc(s.name)}">${I("more_vert")}</button>`:""}</div>`;
  p.innerHTML=`<section aria-labelledby="tH"><div class="ph"><h2 id="tH">Teachers</h2>${owner()?`<button class="ib" id="inviteBtn" aria-label="Invite teachers" style="color:var(--pri)">${I("person_add")}</button>`:""}</div>
   ${A.teachers.map(t=>t.status==="owner"?`<div class="prow"><span class="la" style="background:#5C6BC0" aria-hidden="true">${esc((t.name||"You").trim()[0].toUpperCase())}</span><div class="n">${esc(t.name||"You")}<small>Class owner · can't be removed</small></div><span class="badge b-owner">${I("key")}Owner</span><span style="width:44px" aria-hidden="true"></span></div>`
  ...
  <section aria-labelledby="sH" style="margin-top:24px"><div class="ph"><h2 id="sH">Students</h2><span class="count" id="headcount" aria-live="off">${plural(n,"student")}</span>
   ${owner()?`<button class="ib" id="inviteStudentsBtn" aria-label="Invite students" style="color:var(--pri)">${I("person_add")}</button>
     <button class="ib" id="addStudentBtn" aria-label="Add practice student" style="color:var(--pri)">${I("person")}</button>`:""}</div>
   ${n?A.students.map(srow).join("")
     :`<div class="cw-empty people-empty">${EMPTY_PEOPLE}<b>Add students to this class</b>
        ${owner()?`<div style="margin-top:16px;display:flex;gap:10px;justify-content:center;flex-wrap:wrap"><button class="fb" id="peopleInvite">${I("person_add")}Invite students</button>
        <button class="ob" id="peopleAdd">${I("person")}Add practice student</button></div>`:"<span>Your teacher adds people to this class.</span>"}</div>`}
  </section>
  ...
  [["#inviteStudentsBtn","invite"],["#peopleInvite","invite"],["#addStudentBtn","add"],["#peopleAdd","add"]].forEach(([sel,kind])=>{
    const b=$(sel);if(b)b.onclick=()=>kind==="invite"?inviteStudents(b):openAddStudent(b)});
  ...
  if(owner())$$("[data-student-menu]").forEach(b=>b.onclick=e=>{e.stopPropagation();const s=A.students.find(x=>x.id===b.dataset.studentMenu);
    const items=[];
    if(s.status==="invited")items.push({icon:"check",label:"Accept invitation (practice)",fn:()=>{s.status="active";L.invitesAccepted++;renderPeople(p);snack(`${s.name} joined ${A.name}.`);emit("student:accepted")}});
    items.push({icon:"person_remove",label:"Remove",fn:()=>{A.students=A.students.filter(x=>x!==s);L.studentsRemoved++;renderPeople(p);const f=$("#addStudentBtn")||$("#peopleAdd");f&&f.focus();snack(`Removed ${s.name}. ${plural(A.students.length,"student")} in ${A.name}.`);emit("student:removed")}});
    openMenu(b,items,{alignRight:true})});
}
/* Invite students: the invite link, a searchable contact list, Invite disabled until someone is picked (SPEC §6.8).
   No mail leaves the sandbox — invited people sit as Invited until accepted, as they would in the real product. */
function inviteStudents(trigger){
  const picked=new Set();
  cDialog(`<h2 id="cdT">Invite students</h2>
   <div class="invlink"><div class="k">Invite link</div>
     <div class="row"><span class="url" id="invUrl">https://classroom.google.com/c/${esc(A.id)}?cjc=${esc(A.code)}</span>
     <button class="ib" id="invCopy" aria-label="Copy invite link">${I("content_copy")}</button></div></div>
   <div class="field of srch"><input id="invSearch" autocomplete="off" placeholder=" " aria-describedby="invCount"><label for="invSearch">Type a name or email</label>${I("search","dd")}</div>
   <div class="contacts" id="invList" role="group" aria-label="Contacts"></div>
   <p class="sr" id="invCount" aria-live="polite">No one selected</p>
   <div class="acts"><button class="tb" data-close>Cancel</button><button class="tb" id="invGo" disabled>Invite</button></div>`,
  (d,close)=>{const list=d.querySelector("#invList"),go=d.querySelector("#invGo"),count=d.querySelector("#invCount");
    const taken=new Set(A.students.map(s=>s.name));
    const draw=(q="")=>{const rows=CONTACTS.filter(([n,e])=>!taken.has(n)&&(n+e).toLowerCase().includes(q.toLowerCase()));
      list.innerHTML=rows.length?rows.map(([n,e])=>`<label class="contact"><input type="checkbox" value="${esc(n)}" data-email="${esc(e)}" ${picked.has(n)?"checked":""}>
        <span class="la" style="background:${laColor(n)}" aria-hidden="true">${esc(n[0])}</span><span class="n">${esc(n)}<small>${esc(e)}</small></span></label>`).join("")
        :`<p class="none">No contacts match “${esc(q)}”.</p>`;
      list.querySelectorAll("input").forEach(i=>i.onchange=()=>{i.checked?picked.add(i.value):picked.delete(i.value);
        go.disabled=!picked.size;count.textContent=picked.size?`${plural(picked.size,"person","people")} selected`:"No one selected"})};
    draw();
    d.querySelector("#invSearch").oninput=e=>draw(e.target.value.trim());
    d.querySelector("#invCopy").onclick=()=>snack("Invite link copied (simulated).");
    go.onclick=()=>{picked.forEach(n=>{const c=CONTACTS.find(x=>x[0]===n);
      A.students.push({id:nid("u"),name:n,email:c?c[1]:"",placeholder:false,status:"invited"})});
      L.studentsInvited+=picked.size;close(true);renderPanel();
      snack(`${plural(picked.size,"invitation")} sent (simulated). They stay Invited until they join.`);emit("students:invited")}},trigger);
  emit("invite:students");
}
   ...
   <div class="two">
    <section class="pan" aria-labelledby="csH"><h2 id="csH">${I("domain")}Your class right now</h2><p class="sub">${K?`Current contents of ${esc(K.name)}.`:"You haven't created a class yet."}</p>
     ${K?`<dl class="kv">
      ...
      <dt class="grp">People</dt><dt>Teachers</dt><dd>${K.teachers.filter(t=>t.status==="owner").length} owner${pend?`, ${pend} pending`:""}</dd><dt>Students</dt><dd>${K.students.filter(x=>x.status!=="invited").length} active${K.students.filter(x=>x.status==="invited").length?`, ${K.students.filter(x=>x.status==="invited").length} invited`:""}</dd><dt>Class code</dt><dd style="letter-spacing:.06em">${esc(K.code)}</dd>
     </dl>`:""}
    ...
    <section class="pan" aria-labelledby="aaH"><h2 id="aaH">${I("list_alt")}Actions you tried</h2><p class="sub">Everything you did this session, including work you later removed.</p>
     <dl class="kv">
      ...
      <dt class="grp">People</dt><dt>Co-teacher invitations</dt><dd>${L.invitesSent}</dd><dt>Students invited</dt><dd>${L.studentsInvited}</dd><dt>Invitations accepted</dt><dd>${L.invitesAccepted}</dd><dt>Students added / removed</dt><dd>${L.studentsAdded} / ${L.studentsRemoved}</dd><dt>Class code regenerated</dt><dd>${L.codeRegenerations}</dd>
