...
6. **The class workspace.** Stream, Classwork and People are free to use in any order. The first time each tab opens, a Classroom-style intro card plays a short, silent wireframe animation of that tab's key action. The drawer grows Teaching and Enrolled groups as classes appear, and Home gains a Teaching / Enrolled segmented control once both exist.
